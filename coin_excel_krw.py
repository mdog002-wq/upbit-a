...
def calculate_max_pattern_similarity(series):
    if not series or not GLOBAL_GOLDEN_PATTERN_DATA:
        return 0.0
...
def process_single_coin(item, current_price_map):
    ...
    # 🌐 index.html 호환 키(영문) 및 기존 키(한글) 모두 반환
    return {
        "market": ticker,
        "ticker": symbol,
        "name": korean_name,
...
def generate_gemini_analysis(df_result):
    ...
    qualified_df = df_result[df_result['score'] >= MIN_RECOMMEND_SCORE].head(3)
...
"""
        ...
        for i in raw_recs:
            ...
            if sym:
                match_row = qualified_df[qualified_df["ticker"] == sym]
                entry_price = float(match_row["current_price"].values[0]) if not match_row.empty else 0.0
...
def main():
    ...
    # ------------------------------------------------------------
    # 🌐 대시보드 웹 화면(index.html) 완전 연동용 파일 저장
    # ------------------------------------------------------------
    records = df_res.to_dict(orient="records")

    # 1) docs/dashboard_data.json
    save_json(os.path.join(DOCS_DIR, "dashboard_data.json"), records)

    # 2) docs/history_db.json
    save_json(os.path.join(DOCS_DIR, "history_db.json"), records)

    print("🌐 [대시보드 연동 완료] docs/dashboard_data.json & history_db.json 저장 성공!")
